...
mean <- mean(spiny_counts$counts, na.rm = TRUE)

ggplot(data = spiny_counts, aes(x = counts, y = site)) +
 geom_density_ridges() +
  geom_labelvline(xintercept = mean, color = "red", label = "Mean") +
  labs(color = "Mean")
```
...
# Plot 2: 
library(ggridges)
ggplot(data = spiny_counts, aes(x = mpa)) +
...
```{r}
# Plot 4:
ggplot(data = spiny_counts, aes(x = mpa, y = mean_size)) +
...
lobster_summary %>% 
  as_gt() %>% 
  tab_header(
   title = "Mean Spiny Lobster Counts and Sizes",
...
tab_source_note(
  source_note = "Note: Data from Reed D. 2019, SBC LTER, Environmental Data Initiative"
)
```
...
Yes, because you would still be adding on the year term and the interaction term, which in most cases would make the treatment effect ultimately positive. An exception to this is 2013, but since that was the first year after the MPAs were enacted, it is not completely shocking that the treatment effect is negative.
