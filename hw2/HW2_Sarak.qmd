...
Collaborators: Leela Dixit, Vedika Shirtekar, Ixel Medrano, Jay Kim
...
This paper explores retrospective voting on public policies that divide citizens' preference, specifically using the spatial properties of electoral voting to investigate "spatially distorted signalling." Spatial distorted signalling occurs where there is concentrated, local opposition and diffuse, public support of a certain policy, which signals to the government to adjust a policy that the majority supports. The study leverages a recent climate policy as a natural experiment to test whether citizens retrospectively punish governments for policies with concentrated local costs and diffuse public benefits.

The intervention being evaluated is the expansion of the Green Energy Act in Ontario, Canada, which occurred in 2009. The expansion allowed for corporations, communities, and individuals to build wind turbines and other renewable energy projects throughout the province, signing long-term contracts with the government to sell their energy. The outcome of interest is the effect of the policy on liberal, or incumbent, voting party share.

Unit of analysis: The precinct, where electors cast their vote for their provincial representative at a nearby polling place. The study included 6,186 of these, which made up 26 (out of 107) districts where wind projects were proposed or operational.

Treatment: The two treatments were proposed or operational wind turbines, which came as a result of the Green Energy Act expansion. 

Comparison group: Precincts in the 26 districts that were included in the data set which did not receive the treatment.
...
1. When a turbine is proposed or becomes operational, that may lead to an influx of new residents that hope to find jobs maintaining the wind turbine. It is possible that this influx of residents comes from a specific political background (for example, people who work on wind farms are more likely to be liberal), skewing the results. This is confounding because change in liberal party voting share comes from this new population who came **because** of the treatment, and not from the people who had lived there before.

2. Rural areas tend to have a higher proportion of conservative-minded people. People of an originally liberal viewpoint may find it easier to change their mind when 1) they experience the treatment, and 2) have people around them who already oppose it. This confounds the results because in this scenario liberal party voting share is influenced by neighbors' opinions, instead of just people's own thoughts on the wind turbine.
...
*Response:* The key identifying assumption for a fixed effects / Difference-in-Difference design is parallel trends in the control and treatment groups during the pre-treatment period. This provides evidence of a causal effect because it confirms that, in the absence of treatment, the average outcomes for both groups should follow similar trajectories over time. If they don't, and the only introduced difference is the treatment, we have evidence for causal effect.
...
*Response:* A fixed effects approach allows us to identify the variance within treatment and control groups, and removes variance that occurs due to differences between them. In this study, controlling for precinct and year ensures that changes/differences in liberal party voting share are not caused by precinct or year. An example of this would be if one precinct is historically more liberal than another, or if a significant political event occurred in 2011 that altered all (or some) of the precinct's voting results.

**1B.Q3** What part of the SUTVA assumption is most likely violated in the context of this study design (and why)?

*Response:* **No interference** is most likely violated, because one precinct's wind turbine (treatment) may be visible from another precinct, affecting that precinct's outcome.

**1B.Q4**  Why does spillover matter when estimating an unbiased treatment effect? 

*Response:* Spillover is important because it can obscure or contaminate the identification of the true impact of the treatment. It becomes unclear if the signal is coming solely from the treatment, or other sources. In this case, it becomes unclear if a change in liberal party voting share is coming from the treatment (proposed/operational turbine) or the fact that the precinct nearby received the treatment.
...
*Response:* The study addresses the risk of spillovers by calculating the average treatment effect within precinct groups 1km-increments away from proposed or operational turbines. (Testing the idea that people do not like visible infrastructure). They find that, for both proposed and operational wind turbines, vote share declines can be detected in precincts up to 3 km away from the turbines. For precincts 4 and 5 km away from the turbines, the effects are indistinguishable. This serves as a spillover test, and can be considered in the interpretation of results.
...
*Response:* There is a mismatch between size and number of precincts between years

We are using matching because precincts vary in size and population between the two years. Matching the data allows for us to control for the data for variables like the average home price pretreatment in 2006 (log), the population with a university degree (%), median income (log), and population density (log).
...
balance_table <- match_data %>% select(
...
![](images/balance_table.png)

**2B.Q1** Summarize the table output: Which covariates look balanced/imbalanced?

*Response:* Home value, population with a university degree (%), and median income are look pretty balanced, but there is an imbalance in population density betwee the two years. Another factor is that the number of precincts in the control group is a lot (~16 times) higher than that of the treatment.

**2B.Q2** Describe in your own words why these covariates might be expected to confound the treatment estimate: 

*Response (2-4 sentences):* Variables like university degree, home value, median income, and population density all impact voting behavior. For example, it is often suggested that higher education levels corresponds to more left-leaning beliefs. If there are underlying differences in these variables, regardless of proximity to a proposed turbine, our model might be capturing the effects of those variables, instead of our variable of interest.
...
*Response:* Mahalanobis matching did a better job at achieving balance, because the adjusted differences are smaller (closer to 0) in that model than the propensity score matching model.
...
*Response:* This treatment estimate represents the ATT because it only measures the impact of the treatment on those that received the treatment, not the entire population. This is because our matching method specifically selects controls that are representative of our treatment group. By using the original, un-matched data, a regression model could represent ATE, or the impact of treatment across an entire population, but may do a poorer job of controlling for covariates.
...
*Response:* There are 6186 precincts, and we have observations for all of them across 3 years (2003, 2007, and 2011). 6186, 3 times, is 18,558. These repeating of observations across different time points is characteristic of panel data.
...
*Response:* There are 184 precincts that have had proposed turbines and 52 that have had operational ones, meaning 236 (184 + 52) is the number of precincts that are ever treated in this data. 
...
**Proposed turbines:** The signal-to-noise ratio for the treatment estimate is smaller than 2-to-1. We know this because the standard error of `proposed_turbine` (0.031) is more than 1/2 of the value of its coefficient estimate (|-0.057|). 


**Operational turbines:** The signal-to-noise ratio for the treatment estimate is smaller than 2-to-1. We know this because the standard error of `operational_turbine` (0.054) is more than 1/2 of the value of its coefficient estimate (|-0.106|). 
...
*Response:* Standard error estimates did change: it increased in our model for `proposed_turbine` and decreased in the `operational_turbine` model. Heteroskedasticity occurs when different observations have different error variance. This is most likely true in our model because our observations are not independent -- repeated measurements in one place suggest correlated errors. By adjusting our model to `robust = TRUE`, we make the model treat our errors as correlated, resulting in adjusted standard errors.
...
*Response:* For proposed turbines, our model (run on the sample) predicted that treatment decreased liberal party vote share by ~5.7% while the paper found that treatment decreased liberal party vote share by ~4.2%. For operational turbines, the estimates were a decrease of ~10.6% in our model and ~9.3 in the original study.

I feel that these estimates are relatively similar, which supports a true relationship between the treatment and outcome. However, the estimates do differ slightly, and that is most likely due to 1) our model results being based on a sample of 40 precincts, instead of the entire data, and 2) a potential mismatch in fixed effects estimation package used to achieve our respective results. 
...
*Response (2-4 sentences):* Fixed effects remove omitted variable bias sources from factors that are time-invariant. Specifically, a precinct's location, historical voting patterns, culture, do not vary over time. Therefore, by controlling for the precinct variable, we control for differences between precincts that may affect our outcome (liberal party vote share), ensuring that we only capture the effect of the treatment. Controlling for year absorbs differences/shocks that may have affected all precinct's due to the events of a specific year.

These controls leave only the differences *within* groups in our model, and removes the effect of all differences between groups.
...
*Response:* We are 99% confident that a proposed wind turbine project in a precinct decreases its liberal party vote share by ~4%, on average.
...
*Response:* We are 99% confidence that the presence of operational wind turbines in a precinct decreases liberal party vote share by ~9%, on average. 
...
*Response:* The effect of proposed wind turbines is different from operational wind turbines because present wind turbines have an actual, immediate, and visual effect on precinct residents. The difference in effect comes from the difference in imagining versus having to actually exist with and see a wind turbine in your neighborhood; precinct voters are more likely to disapprove of wind turbines when they are in plain sight than when they are just an idea. 
Stokes (2015) references this when discussing the spillover effect, saying that effect may be related to proximity to unwanted infrastructure. And in the conclusion: "This result is intuitive: When projects become operational, more citizens know about the project and experience its impacts."
